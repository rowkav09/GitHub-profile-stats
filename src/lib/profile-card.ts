...
function repoLayout(data: ProfileCardData, options: ProfileCardOptions) {
  ...
  const heading = escapeXml(options.title || `${data.name || data.username}'s GitHub profile`);
  const subtitle = escapeXml(options.subtitle || "Live open-source activity, generated by ghstats.dev.");
  return `<rect width="${width}" height="${height}" fill="${theme.bg}"/><rect x="1" y="1" width="${width - 2}" height="${height - 2}" rx="28" fill="${theme.bg}" stroke="${theme.border}" stroke-width="2"/>${options.showAvatar ? avatar(data, width - 250, 62, 172, theme) : ""}<text x="70" y="115" font-family="${font}" font-size="34" fill="${theme.text}"><tspan font-weight="400">${escapeXml(data.username)}/</tspan><tspan font-weight="700">GitHub profile</tspan></text><text x="70" y="165" font-family="${font}" font-size="22" fill="${theme.muted}">${subtitle}</text>${stat(70, 342, data.followers, "Followers", theme.text, theme.muted)}${stat(250, 342, data.publicRepos, "Public repos", theme.text, theme.muted)}${stat(430, 342, data.totalStars, "Stars earned", theme.text, theme.muted)}${stat(630, 342, data.contributionsThisYear, "Contributions", theme.text, theme.muted)}${stat(870, 342, data.currentStreak, "Day streak", theme.text, theme.muted)}${options.showLanguages ? languagePills(data, 445, theme) : ""}<rect x="0" y="${height - 18}" width="${width}" height="18" fill="${theme.accent}"/><text x="${width - 70}" y="${height - 42}" text-anchor="end" font-family="${font}" font-size="16" fill="${theme.muted}">ghstats.dev/${escapeXml(data.username)}</text>`;
}
