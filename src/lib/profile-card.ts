...
function repoLayout(data: ProfileCardData, options: ProfileCardOptions) {
  ...
  const bioLine = bio ? `<text x="70" y="172" font-family="${font}" font-size="20" fill="${theme.muted}">${escapeXml(bio.slice(0, 110))}</text>` : "";
  return `<rect width="${width}" height="${height}" fill="${theme.bg}"/><rect x="1" y="1" width="${width - 2}" height="${height - 2}" rx="28" fill="${theme.bg}" stroke="${theme.border}" stroke-width="2"/>${options.showAvatar ? avatar(data, width - 250, 62, 172, theme) : ""}<text x="70" y="112" font-family="${font}" font-size="38" font-weight="700" fill="${theme.text}">${displayName}</text><text x="70" y="145" font-family="${font}" font-size="21" fill="${theme.accent}">@${escapeXml(data.username)}</text>${bioLine}${stat(70, 342, data.followers, "Followers", theme.text, theme.muted)}${stat(250, 342, data.publicRepos, "Public repos", theme.text, theme.muted)}${stat(430, 342, data.totalStars, "Stars earned", theme.text, theme.muted)}${stat(630, 342, data.contributionsThisYear, "Contributions", theme.text, theme.muted)}${stat(870, 342, data.currentStreak, "Day streak", theme.text, theme.muted)}${options.showLanguages ? languagePills(data, 445, theme) : ""}<rect x="0" y="${height - 18}" width="${width}" height="18" fill="${theme.accent}"/>`;
}

function profileLayout(data: ProfileCardData, options: ProfileCardOptions) {
  const { theme, width, height } = options;
  const bio = options.subtitle ?? data.bio ?? "";
  const bioLine = bio ? `<text x="305" y="235" font-family="${font}" font-size="19" fill="${theme.muted}">${escapeXml(bio.slice(0, 100))}</text>` : "";
  return `<rect width="${width}" height="${height}" fill="${theme.bg}"/><rect x="38" y="38" width="${width - 76}" height="${height - 76}" rx="34" fill="${theme.panel}" stroke="${theme.border}" stroke-width="2"/>${options.showAvatar ? avatar(data, 78, 88, 190, theme) : ""}<text x="305" y="145" font-family="${font}" font-size="46" font-weight="700" fill="${theme.text}">${escapeXml(data.name || data.username)}</text><text x="305" y="190" font-family="${font}" font-size="24" fill="${theme.accent}">@${escapeXml(data.username)}</text>${bioLine}${stat(80, 390, data.contributionsThisYear, "Contributions this year", theme.text, theme.muted)}${stat(365, 390, data.totalStars, "Stars earned", theme.text, theme.muted)}${stat(575, 390, data.totalPRs, "Pull requests", theme.text, theme.muted)}${stat(785, 390, data.followers, "Followers", theme.text, theme.muted)}${options.showLanguages ? languagePills(data, 485, theme) : ""}`;
}

function compactLayout(data: ProfileCardData, options: ProfileCardOptions) {
  const { theme, width, height } = options;
  return `<rect width="${width}" height="${height}" fill="${theme.bg}"/><rect x="1" y="1" width="${width - 2}" height="${height - 2}" rx="24" fill="${theme.panel}" stroke="${theme.border}" stroke-width="2"/>${options.showAvatar ? avatar(data, 48, 48, 140, theme) : ""}<text x="225" y="93" font-family="${font}" font-size="34" font-weight="700" fill="${theme.text}">${escapeXml(data.name || data.username)}</text><text x="225" y="132" font-family="${font}" font-size="20" fill="${theme.accent}">@${escapeXml(data.username)}</text>${stat(225, 220, data.followers, "Followers", theme.text, theme.muted)}${stat(400, 220, data.totalStars, "Stars", theme.text, theme.muted)}${stat(550, 220, data.publicRepos, "Repos", theme.text, theme.muted)}${stat(700, 220, data.contributionsThisYear, "Contributions", theme.text, theme.muted)}`;
}
