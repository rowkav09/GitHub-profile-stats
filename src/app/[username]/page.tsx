...
export function generateMetadata({ params }: ProfilePageProps): Metadata {
  ...
  const cardImage = `${SITE.url}${SITE_ROUTES.apiCard}?username=${encodeURIComponent(username)}`;
...
export default function ProfilePage({ params }: ProfilePageProps) {
  ...
  const absoluteCardUrl = `${SITE.url}${cardUrl}`;
  const markdown = `![${username}'s GitHub stats](${absoluteCardUrl})`;
  ...
  return (
    <main className="min-h-screen bg-[#0d1117]">
      ...
      <section className="mx-auto max-w-5xl space-y-8 px-6 py-12">
        <article className="rounded-2xl border border-[#30363d] bg-[#010409] p-5 shadow-2xl shadow-black/20 sm:p-8">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold">Profile stats</h2>
              <p className="mt-1 text-sm text-[#8b949e]">Contributions, activity, streaks, repos, and community stats.</p>
            </div>
            <a
              href={cardUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg border border-[#30363d] px-3 py-2 text-xs font-semibold text-[#c9d1d9] transition-colors hover:border-[#58a6ff]"
            >
              Open SVG
            </a>
          </div>
          ...
          <img
            src={cardUrl}
            alt={`${username}'s GitHub stats card`}
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.72fr)]">
          <article className="rounded-2xl border border-[#30363d] bg-[#010409] p-5 sm:p-8">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-[#8b949e]">Calculated across public, non-fork repositories.</p>
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={languagesUrl}
          ...
          <aside className="rounded-2xl border border-[#30363d] bg-[#161b22] p-5 sm:p-8">
            ...
            <p className="mt-2 text-sm text-[#8b949e]">
              Paste this line into a GitHub profile README. It stays linked to the live card.
            </p>
            <pre className="mt-5 overflow-x-auto whitespace-pre-wrap break-all rounded-lg border border-[#30363d] bg-[#0d1117] p-4 text-xs leading-6 text-[#79c0ff]">
              {markdown}
            </pre>
            <div className="mt-5 flex flex-col gap-3 sm:flex-row lg:flex-col">
              <Link
                href={`${SITE_ROUTES.home}#try`}
                className="rounded-lg bg-[#238636] px-4 py-2.5 text-center text-sm font-semibold text-white transition-colors hover:bg-[#2ea043]"
              >
                Customise the card
              </Link>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-lg border border-[#30363d] px-4 py-2.5 text-center text-sm font-semibold text-[#c9d1d9] transition-colors hover:border-[#8b949e]"
              >
                View GitHub profile
              </a>
            </div>
